...
VectorIndex::LoadIndex(const std::string& p_loaderFilePath, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    LOG(Helper::LogLevel::LL_Info, "Index Path: %s\n", folderPath.c_str());
    // exit(1);

    LOG(Helper::LogLevel::LL_Info, "Reading ini\n");
    ...
    LOG(Helper::LogLevel::LL_Info, "Analyze ini & build index\n");
    ...
    ErrorCode ret = ErrorCode::Success;
    LOG(Helper::LogLevel::LL_Info, "Load Config\n");
